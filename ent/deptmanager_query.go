// Code generated by ent, DO NOT EDIT.

package ent

import (
	"context"
	"employees/ent/department"
	"employees/ent/deptmanager"
	"employees/ent/employee"
	"employees/ent/predicate"
	"fmt"
	"math"

	"entgo.io/ent/dialect/sql"
	"entgo.io/ent/dialect/sql/sqlgraph"
	"entgo.io/ent/schema/field"
)

// DeptManagerQuery is the builder for querying DeptManager entities.
type DeptManagerQuery struct {
	config
	ctx            *QueryContext
	order          []deptmanager.OrderOption
	inters         []Interceptor
	predicates     []predicate.DeptManager
	withEmployee   *EmployeeQuery
	withDepartment *DepartmentQuery
	// intermediate query (i.e. traversal path).
	sql  *sql.Selector
	path func(context.Context) (*sql.Selector, error)
}

// Where adds a new predicate for the DeptManagerQuery builder.
func (dmq *DeptManagerQuery) Where(ps ...predicate.DeptManager) *DeptManagerQuery {
	dmq.predicates = append(dmq.predicates, ps...)
	return dmq
}

// Limit the number of records to be returned by this query.
func (dmq *DeptManagerQuery) Limit(limit int) *DeptManagerQuery {
	dmq.ctx.Limit = &limit
	return dmq
}

// Offset to start from.
func (dmq *DeptManagerQuery) Offset(offset int) *DeptManagerQuery {
	dmq.ctx.Offset = &offset
	return dmq
}

// Unique configures the query builder to filter duplicate records on query.
// By default, unique is set to true, and can be disabled using this method.
func (dmq *DeptManagerQuery) Unique(unique bool) *DeptManagerQuery {
	dmq.ctx.Unique = &unique
	return dmq
}

// Order specifies how the records should be ordered.
func (dmq *DeptManagerQuery) Order(o ...deptmanager.OrderOption) *DeptManagerQuery {
	dmq.order = append(dmq.order, o...)
	return dmq
}

// QueryEmployee chains the current query on the "employee" edge.
func (dmq *DeptManagerQuery) QueryEmployee() *EmployeeQuery {
	query := (&EmployeeClient{config: dmq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := dmq.prepareQuery(ctx); err != nil {
			return nil, err
		}
		selector := dmq.sqlQuery(ctx)
		if err := selector.Err(); err != nil {
			return nil, err
		}
		step := sqlgraph.NewStep(
			sqlgraph.From(deptmanager.Table, deptmanager.FieldID, selector),
			sqlgraph.To(employee.Table, employee.FieldID),
			sqlgraph.Edge(sqlgraph.M2O, true, deptmanager.EmployeeTable, deptmanager.EmployeeColumn),
		)
		fromU = sqlgraph.SetNeighbors(dmq.driver.Dialect(), step)
		return fromU, nil
	}
	return query
}

// QueryDepartment chains the current query on the "department" edge.
func (dmq *DeptManagerQuery) QueryDepartment() *DepartmentQuery {
	query := (&DepartmentClient{config: dmq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := dmq.prepareQuery(ctx); err != nil {
			return nil, err
		}
		selector := dmq.sqlQuery(ctx)
		if err := selector.Err(); err != nil {
			return nil, err
		}
		step := sqlgraph.NewStep(
			sqlgraph.From(deptmanager.Table, deptmanager.FieldID, selector),
			sqlgraph.To(department.Table, department.FieldID),
			sqlgraph.Edge(sqlgraph.M2O, true, deptmanager.DepartmentTable, deptmanager.DepartmentColumn),
		)
		fromU = sqlgraph.SetNeighbors(dmq.driver.Dialect(), step)
		return fromU, nil
	}
	return query
}

// First returns the first DeptManager entity from the query.
// Returns a *NotFoundError when no DeptManager was found.
func (dmq *DeptManagerQuery) First(ctx context.Context) (*DeptManager, error) {
	nodes, err := dmq.Limit(1).All(setContextOp(ctx, dmq.ctx, "First"))
	if err != nil {
		return nil, err
	}
	if len(nodes) == 0 {
		return nil, &NotFoundError{deptmanager.Label}
	}
	return nodes[0], nil
}

// FirstX is like First, but panics if an error occurs.
func (dmq *DeptManagerQuery) FirstX(ctx context.Context) *DeptManager {
	node, err := dmq.First(ctx)
	if err != nil && !IsNotFound(err) {
		panic(err)
	}
	return node
}

// FirstID returns the first DeptManager ID from the query.
// Returns a *NotFoundError when no DeptManager ID was found.
func (dmq *DeptManagerQuery) FirstID(ctx context.Context) (id string, err error) {
	var ids []string
	if ids, err = dmq.Limit(1).IDs(setContextOp(ctx, dmq.ctx, "FirstID")); err != nil {
		return
	}
	if len(ids) == 0 {
		err = &NotFoundError{deptmanager.Label}
		return
	}
	return ids[0], nil
}

// FirstIDX is like FirstID, but panics if an error occurs.
func (dmq *DeptManagerQuery) FirstIDX(ctx context.Context) string {
	id, err := dmq.FirstID(ctx)
	if err != nil && !IsNotFound(err) {
		panic(err)
	}
	return id
}

// Only returns a single DeptManager entity found by the query, ensuring it only returns one.
// Returns a *NotSingularError when more than one DeptManager entity is found.
// Returns a *NotFoundError when no DeptManager entities are found.
func (dmq *DeptManagerQuery) Only(ctx context.Context) (*DeptManager, error) {
	nodes, err := dmq.Limit(2).All(setContextOp(ctx, dmq.ctx, "Only"))
	if err != nil {
		return nil, err
	}
	switch len(nodes) {
	case 1:
		return nodes[0], nil
	case 0:
		return nil, &NotFoundError{deptmanager.Label}
	default:
		return nil, &NotSingularError{deptmanager.Label}
	}
}

// OnlyX is like Only, but panics if an error occurs.
func (dmq *DeptManagerQuery) OnlyX(ctx context.Context) *DeptManager {
	node, err := dmq.Only(ctx)
	if err != nil {
		panic(err)
	}
	return node
}

// OnlyID is like Only, but returns the only DeptManager ID in the query.
// Returns a *NotSingularError when more than one DeptManager ID is found.
// Returns a *NotFoundError when no entities are found.
func (dmq *DeptManagerQuery) OnlyID(ctx context.Context) (id string, err error) {
	var ids []string
	if ids, err = dmq.Limit(2).IDs(setContextOp(ctx, dmq.ctx, "OnlyID")); err != nil {
		return
	}
	switch len(ids) {
	case 1:
		id = ids[0]
	case 0:
		err = &NotFoundError{deptmanager.Label}
	default:
		err = &NotSingularError{deptmanager.Label}
	}
	return
}

// OnlyIDX is like OnlyID, but panics if an error occurs.
func (dmq *DeptManagerQuery) OnlyIDX(ctx context.Context) string {
	id, err := dmq.OnlyID(ctx)
	if err != nil {
		panic(err)
	}
	return id
}

// All executes the query and returns a list of DeptManagers.
func (dmq *DeptManagerQuery) All(ctx context.Context) ([]*DeptManager, error) {
	ctx = setContextOp(ctx, dmq.ctx, "All")
	if err := dmq.prepareQuery(ctx); err != nil {
		return nil, err
	}
	qr := querierAll[[]*DeptManager, *DeptManagerQuery]()
	return withInterceptors[[]*DeptManager](ctx, dmq, qr, dmq.inters)
}

// AllX is like All, but panics if an error occurs.
func (dmq *DeptManagerQuery) AllX(ctx context.Context) []*DeptManager {
	nodes, err := dmq.All(ctx)
	if err != nil {
		panic(err)
	}
	return nodes
}

// IDs executes the query and returns a list of DeptManager IDs.
func (dmq *DeptManagerQuery) IDs(ctx context.Context) (ids []string, err error) {
	if dmq.ctx.Unique == nil && dmq.path != nil {
		dmq.Unique(true)
	}
	ctx = setContextOp(ctx, dmq.ctx, "IDs")
	if err = dmq.Select(deptmanager.FieldID).Scan(ctx, &ids); err != nil {
		return nil, err
	}
	return ids, nil
}

// IDsX is like IDs, but panics if an error occurs.
func (dmq *DeptManagerQuery) IDsX(ctx context.Context) []string {
	ids, err := dmq.IDs(ctx)
	if err != nil {
		panic(err)
	}
	return ids
}

// Count returns the count of the given query.
func (dmq *DeptManagerQuery) Count(ctx context.Context) (int, error) {
	ctx = setContextOp(ctx, dmq.ctx, "Count")
	if err := dmq.prepareQuery(ctx); err != nil {
		return 0, err
	}
	return withInterceptors[int](ctx, dmq, querierCount[*DeptManagerQuery](), dmq.inters)
}

// CountX is like Count, but panics if an error occurs.
func (dmq *DeptManagerQuery) CountX(ctx context.Context) int {
	count, err := dmq.Count(ctx)
	if err != nil {
		panic(err)
	}
	return count
}

// Exist returns true if the query has elements in the graph.
func (dmq *DeptManagerQuery) Exist(ctx context.Context) (bool, error) {
	ctx = setContextOp(ctx, dmq.ctx, "Exist")
	switch _, err := dmq.FirstID(ctx); {
	case IsNotFound(err):
		return false, nil
	case err != nil:
		return false, fmt.Errorf("ent: check existence: %w", err)
	default:
		return true, nil
	}
}

// ExistX is like Exist, but panics if an error occurs.
func (dmq *DeptManagerQuery) ExistX(ctx context.Context) bool {
	exist, err := dmq.Exist(ctx)
	if err != nil {
		panic(err)
	}
	return exist
}

// Clone returns a duplicate of the DeptManagerQuery builder, including all associated steps. It can be
// used to prepare common query builders and use them differently after the clone is made.
func (dmq *DeptManagerQuery) Clone() *DeptManagerQuery {
	if dmq == nil {
		return nil
	}
	return &DeptManagerQuery{
		config:         dmq.config,
		ctx:            dmq.ctx.Clone(),
		order:          append([]deptmanager.OrderOption{}, dmq.order...),
		inters:         append([]Interceptor{}, dmq.inters...),
		predicates:     append([]predicate.DeptManager{}, dmq.predicates...),
		withEmployee:   dmq.withEmployee.Clone(),
		withDepartment: dmq.withDepartment.Clone(),
		// clone intermediate query.
		sql:  dmq.sql.Clone(),
		path: dmq.path,
	}
}

// WithEmployee tells the query-builder to eager-load the nodes that are connected to
// the "employee" edge. The optional arguments are used to configure the query builder of the edge.
func (dmq *DeptManagerQuery) WithEmployee(opts ...func(*EmployeeQuery)) *DeptManagerQuery {
	query := (&EmployeeClient{config: dmq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	dmq.withEmployee = query
	return dmq
}

// WithDepartment tells the query-builder to eager-load the nodes that are connected to
// the "department" edge. The optional arguments are used to configure the query builder of the edge.
func (dmq *DeptManagerQuery) WithDepartment(opts ...func(*DepartmentQuery)) *DeptManagerQuery {
	query := (&DepartmentClient{config: dmq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	dmq.withDepartment = query
	return dmq
}

// GroupBy is used to group vertices by one or more fields/columns.
// It is often used with aggregate functions, like: count, max, mean, min, sum.
//
// Example:
//
//	var v []struct {
//		EmployeeID string `json:"employee_id,omitempty"`
//		Count int `json:"count,omitempty"`
//	}
//
//	client.DeptManager.Query().
//		GroupBy(deptmanager.FieldEmployeeID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
func (dmq *DeptManagerQuery) GroupBy(field string, fields ...string) *DeptManagerGroupBy {
	dmq.ctx.Fields = append([]string{field}, fields...)
	grbuild := &DeptManagerGroupBy{build: dmq}
	grbuild.flds = &dmq.ctx.Fields
	grbuild.label = deptmanager.Label
	grbuild.scan = grbuild.Scan
	return grbuild
}

// Select allows the selection one or more fields/columns for the given query,
// instead of selecting all fields in the entity.
//
// Example:
//
//	var v []struct {
//		EmployeeID string `json:"employee_id,omitempty"`
//	}
//
//	client.DeptManager.Query().
//		Select(deptmanager.FieldEmployeeID).
//		Scan(ctx, &v)
func (dmq *DeptManagerQuery) Select(fields ...string) *DeptManagerSelect {
	dmq.ctx.Fields = append(dmq.ctx.Fields, fields...)
	sbuild := &DeptManagerSelect{DeptManagerQuery: dmq}
	sbuild.label = deptmanager.Label
	sbuild.flds, sbuild.scan = &dmq.ctx.Fields, sbuild.Scan
	return sbuild
}

// Aggregate returns a DeptManagerSelect configured with the given aggregations.
func (dmq *DeptManagerQuery) Aggregate(fns ...AggregateFunc) *DeptManagerSelect {
	return dmq.Select().Aggregate(fns...)
}

func (dmq *DeptManagerQuery) prepareQuery(ctx context.Context) error {
	for _, inter := range dmq.inters {
		if inter == nil {
			return fmt.Errorf("ent: uninitialized interceptor (forgotten import ent/runtime?)")
		}
		if trv, ok := inter.(Traverser); ok {
			if err := trv.Traverse(ctx, dmq); err != nil {
				return err
			}
		}
	}
	for _, f := range dmq.ctx.Fields {
		if !deptmanager.ValidColumn(f) {
			return &ValidationError{Name: f, err: fmt.Errorf("ent: invalid field %q for query", f)}
		}
	}
	if dmq.path != nil {
		prev, err := dmq.path(ctx)
		if err != nil {
			return err
		}
		dmq.sql = prev
	}
	return nil
}

func (dmq *DeptManagerQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*DeptManager, error) {
	var (
		nodes       = []*DeptManager{}
		_spec       = dmq.querySpec()
		loadedTypes = [2]bool{
			dmq.withEmployee != nil,
			dmq.withDepartment != nil,
		}
	)
	_spec.ScanValues = func(columns []string) ([]any, error) {
		return (*DeptManager).scanValues(nil, columns)
	}
	_spec.Assign = func(columns []string, values []any) error {
		node := &DeptManager{config: dmq.config}
		nodes = append(nodes, node)
		node.Edges.loadedTypes = loadedTypes
		return node.assignValues(columns, values)
	}
	for i := range hooks {
		hooks[i](ctx, _spec)
	}
	if err := sqlgraph.QueryNodes(ctx, dmq.driver, _spec); err != nil {
		return nil, err
	}
	if len(nodes) == 0 {
		return nodes, nil
	}
	if query := dmq.withEmployee; query != nil {
		if err := dmq.loadEmployee(ctx, query, nodes, nil,
			func(n *DeptManager, e *Employee) { n.Edges.Employee = e }); err != nil {
			return nil, err
		}
	}
	if query := dmq.withDepartment; query != nil {
		if err := dmq.loadDepartment(ctx, query, nodes, nil,
			func(n *DeptManager, e *Department) { n.Edges.Department = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
}

func (dmq *DeptManagerQuery) loadEmployee(ctx context.Context, query *EmployeeQuery, nodes []*DeptManager, init func(*DeptManager), assign func(*DeptManager, *Employee)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*DeptManager)
	for i := range nodes {
		fk := nodes[i].EmployeeID
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	}
	if len(ids) == 0 {
		return nil
	}
	query.Where(employee.IDIn(ids...))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "employee_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
	}
	return nil
}
func (dmq *DeptManagerQuery) loadDepartment(ctx context.Context, query *DepartmentQuery, nodes []*DeptManager, init func(*DeptManager), assign func(*DeptManager, *Department)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*DeptManager)
	for i := range nodes {
		fk := nodes[i].DepartmentID
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	}
	if len(ids) == 0 {
		return nil
	}
	query.Where(department.IDIn(ids...))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "department_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
	}
	return nil
}

func (dmq *DeptManagerQuery) sqlCount(ctx context.Context) (int, error) {
	_spec := dmq.querySpec()
	_spec.Node.Columns = dmq.ctx.Fields
	if len(dmq.ctx.Fields) > 0 {
		_spec.Unique = dmq.ctx.Unique != nil && *dmq.ctx.Unique
	}
	return sqlgraph.CountNodes(ctx, dmq.driver, _spec)
}

func (dmq *DeptManagerQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(deptmanager.Table, deptmanager.Columns, sqlgraph.NewFieldSpec(deptmanager.FieldID, field.TypeString))
	_spec.From = dmq.sql
	if unique := dmq.ctx.Unique; unique != nil {
		_spec.Unique = *unique
	} else if dmq.path != nil {
		_spec.Unique = true
	}
	if fields := dmq.ctx.Fields; len(fields) > 0 {
		_spec.Node.Columns = make([]string, 0, len(fields))
		_spec.Node.Columns = append(_spec.Node.Columns, deptmanager.FieldID)
		for i := range fields {
			if fields[i] != deptmanager.FieldID {
				_spec.Node.Columns = append(_spec.Node.Columns, fields[i])
			}
		}
		if dmq.withEmployee != nil {
			_spec.Node.AddColumnOnce(deptmanager.FieldEmployeeID)
		}
		if dmq.withDepartment != nil {
			_spec.Node.AddColumnOnce(deptmanager.FieldDepartmentID)
		}
	}
	if ps := dmq.predicates; len(ps) > 0 {
		_spec.Predicate = func(selector *sql.Selector) {
			for i := range ps {
				ps[i](selector)
			}
		}
	}
	if limit := dmq.ctx.Limit; limit != nil {
		_spec.Limit = *limit
	}
	if offset := dmq.ctx.Offset; offset != nil {
		_spec.Offset = *offset
	}
	if ps := dmq.order; len(ps) > 0 {
		_spec.Order = func(selector *sql.Selector) {
			for i := range ps {
				ps[i](selector)
			}
		}
	}
	return _spec
}

func (dmq *DeptManagerQuery) sqlQuery(ctx context.Context) *sql.Selector {
	builder := sql.Dialect(dmq.driver.Dialect())
	t1 := builder.Table(deptmanager.Table)
	columns := dmq.ctx.Fields
	if len(columns) == 0 {
		columns = deptmanager.Columns
	}
	selector := builder.Select(t1.Columns(columns...)...).From(t1)
	if dmq.sql != nil {
		selector = dmq.sql
		selector.Select(selector.Columns(columns...)...)
	}
	if dmq.ctx.Unique != nil && *dmq.ctx.Unique {
		selector.Distinct()
	}
	for _, p := range dmq.predicates {
		p(selector)
	}
	for _, p := range dmq.order {
		p(selector)
	}
	if offset := dmq.ctx.Offset; offset != nil {
		// limit is mandatory for offset clause. We start
		// with default value, and override it below if needed.
		selector.Offset(*offset).Limit(math.MaxInt32)
	}
	if limit := dmq.ctx.Limit; limit != nil {
		selector.Limit(*limit)
	}
	return selector
}

// DeptManagerGroupBy is the group-by builder for DeptManager entities.
type DeptManagerGroupBy struct {
	selector
	build *DeptManagerQuery
}

// Aggregate adds the given aggregation functions to the group-by query.
func (dmgb *DeptManagerGroupBy) Aggregate(fns ...AggregateFunc) *DeptManagerGroupBy {
	dmgb.fns = append(dmgb.fns, fns...)
	return dmgb
}

// Scan applies the selector query and scans the result into the given value.
func (dmgb *DeptManagerGroupBy) Scan(ctx context.Context, v any) error {
	ctx = setContextOp(ctx, dmgb.build.ctx, "GroupBy")
	if err := dmgb.build.prepareQuery(ctx); err != nil {
		return err
	}
	return scanWithInterceptors[*DeptManagerQuery, *DeptManagerGroupBy](ctx, dmgb.build, dmgb, dmgb.build.inters, v)
}

func (dmgb *DeptManagerGroupBy) sqlScan(ctx context.Context, root *DeptManagerQuery, v any) error {
	selector := root.sqlQuery(ctx).Select()
	aggregation := make([]string, 0, len(dmgb.fns))
	for _, fn := range dmgb.fns {
		aggregation = append(aggregation, fn(selector))
	}
	if len(selector.SelectedColumns()) == 0 {
		columns := make([]string, 0, len(*dmgb.flds)+len(dmgb.fns))
		for _, f := range *dmgb.flds {
			columns = append(columns, selector.C(f))
		}
		columns = append(columns, aggregation...)
		selector.Select(columns...)
	}
	selector.GroupBy(selector.Columns(*dmgb.flds...)...)
	if err := selector.Err(); err != nil {
		return err
	}
	rows := &sql.Rows{}
	query, args := selector.Query()
	if err := dmgb.build.driver.Query(ctx, query, args, rows); err != nil {
		return err
	}
	defer rows.Close()
	return sql.ScanSlice(rows, v)
}

// DeptManagerSelect is the builder for selecting fields of DeptManager entities.
type DeptManagerSelect struct {
	*DeptManagerQuery
	selector
}

// Aggregate adds the given aggregation functions to the selector query.
func (dms *DeptManagerSelect) Aggregate(fns ...AggregateFunc) *DeptManagerSelect {
	dms.fns = append(dms.fns, fns...)
	return dms
}

// Scan applies the selector query and scans the result into the given value.
func (dms *DeptManagerSelect) Scan(ctx context.Context, v any) error {
	ctx = setContextOp(ctx, dms.ctx, "Select")
	if err := dms.prepareQuery(ctx); err != nil {
		return err
	}
	return scanWithInterceptors[*DeptManagerQuery, *DeptManagerSelect](ctx, dms.DeptManagerQuery, dms, dms.inters, v)
}

func (dms *DeptManagerSelect) sqlScan(ctx context.Context, root *DeptManagerQuery, v any) error {
	selector := root.sqlQuery(ctx)
	aggregation := make([]string, 0, len(dms.fns))
	for _, fn := range dms.fns {
		aggregation = append(aggregation, fn(selector))
	}
	switch n := len(*dms.selector.flds); {
	case n == 0 && len(aggregation) > 0:
		selector.Select(aggregation...)
	case n != 0 && len(aggregation) > 0:
		selector.AppendSelect(aggregation...)
	}
	rows := &sql.Rows{}
	query, args := selector.Query()
	if err := dms.driver.Query(ctx, query, args, rows); err != nil {
		return err
	}
	defer rows.Close()
	return sql.ScanSlice(rows, v)
}
