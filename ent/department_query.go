...
func (dq *DepartmentQuery) QueryDeptEmployees() *DeptEmployeeQuery {
	query := (&DeptEmployeeClient{config: dq.config}).Query()
...
func (dq *DepartmentQuery) QueryDeptManagers() *DeptManagerQuery {
	query := (&DeptManagerClient{config: dq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := dq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (dq *DepartmentQuery) Clone() *DepartmentQuery {
	...
	return &DepartmentQuery{
		...
		withDeptEmployees: dq.withDeptEmployees.Clone(),
		withDeptManagers:  dq.withDeptManagers.Clone(),
...
func (dq *DepartmentQuery) WithDeptEmployees(opts ...func(*DeptEmployeeQuery)) *DepartmentQuery {
	query := (&DeptEmployeeClient{config: dq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	dq.withDeptEmployees = query
	return dq
}
...
func (dq *DepartmentQuery) WithDeptManagers(opts ...func(*DeptManagerQuery)) *DepartmentQuery {
	query := (&DeptManagerClient{config: dq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	dq.withDeptManagers = query
	return dq
}
...
func (dq *DepartmentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Department, error) {
	var (
		...
		_spec       = dq.querySpec()
		loadedTypes = [2]bool{
			dq.withDeptEmployees != nil,
			dq.withDeptManagers != nil,
		}
	)
	...
	if query := dq.withDeptEmployees; query != nil {
		if err := dq.loadDeptEmployees(ctx, query, nodes,
			func(n *Department) { n.Edges.DeptEmployees = []*DeptEmployee{} },
			func(n *Department, e *DeptEmployee) { n.Edges.DeptEmployees = append(n.Edges.DeptEmployees, e) }); err != nil {
			return nil, err
		}
	}
	if query := dq.withDeptManagers; query != nil {
		if err := dq.loadDeptManagers(ctx, query, nodes,
			func(n *Department) { n.Edges.DeptManagers = []*DeptManager{} },
			func(n *Department, e *DeptManager) { n.Edges.DeptManagers = append(n.Edges.DeptManagers, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (dq *DepartmentQuery) loadDeptEmployees(ctx context.Context, query *DeptEmployeeQuery, nodes []*Department, init func(*Department), assign func(*Department, *DeptEmployee)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(deptemployee.FieldDepartmentID)
	}
	query.Where(predicate.DeptEmployee(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(department.DeptEmployeesColumn), fks...))
	}))
...
func (dq *DepartmentQuery) loadDeptManagers(ctx context.Context, query *DeptManagerQuery, nodes []*Department, init func(*Department), assign func(*Department, *DeptManager)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*Department)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(deptmanager.FieldDepartmentID)
	}
	query.Where(predicate.DeptManager(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(department.DeptManagersColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
