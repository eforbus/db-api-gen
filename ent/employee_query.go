...
type EmployeeQuery struct {
	...
	withDeptEmployees *DeptEmployeeQuery
	withDeptManagers  *DeptManagerQuery
	withSalaries      *SalaryQuery
...
// QueryDeptEmployees chains the current query on the "dept_employees" edge.
func (eq *EmployeeQuery) QueryDeptEmployees() *DeptEmployeeQuery {
	query := (&DeptEmployeeClient{config: eq.config}).Query()
...
// QueryDeptManagers chains the current query on the "dept_managers" edge.
func (eq *EmployeeQuery) QueryDeptManagers() *DeptManagerQuery {
	query := (&DeptManagerClient{config: eq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := eq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// QuerySalaries chains the current query on the "salaries" edge.
func (eq *EmployeeQuery) QuerySalaries() *SalaryQuery {
	query := (&SalaryClient{config: eq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := eq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (eq *EmployeeQuery) QueryTitles() *TitleQuery {
	query := (&TitleClient{config: eq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := eq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (eq *EmployeeQuery) Clone() *EmployeeQuery {
	...
	return &EmployeeQuery{
		...
		withDeptEmployees: eq.withDeptEmployees.Clone(),
		withDeptManagers:  eq.withDeptManagers.Clone(),
		withSalaries:      eq.withSalaries.Clone(),
		withTitles:        eq.withTitles.Clone(),
...
// WithDeptEmployees tells the query-builder to eager-load the nodes that are connected to
// the "dept_employees" edge. The optional arguments are used to configure the query builder of the edge.
func (eq *EmployeeQuery) WithDeptEmployees(opts ...func(*DeptEmployeeQuery)) *EmployeeQuery {
	query := (&DeptEmployeeClient{config: eq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	eq.withDeptEmployees = query
	return eq
}

// WithDeptManagers tells the query-builder to eager-load the nodes that are connected to
// the "dept_managers" edge. The optional arguments are used to configure the query builder of the edge.
func (eq *EmployeeQuery) WithDeptManagers(opts ...func(*DeptManagerQuery)) *EmployeeQuery {
	query := (&DeptManagerClient{config: eq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	eq.withDeptManagers = query
	return eq
}

// WithSalaries tells the query-builder to eager-load the nodes that are connected to
// the "salaries" edge. The optional arguments are used to configure the query builder of the edge.
func (eq *EmployeeQuery) WithSalaries(opts ...func(*SalaryQuery)) *EmployeeQuery {
	query := (&SalaryClient{config: eq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	eq.withSalaries = query
	return eq
}

// WithTitles tells the query-builder to eager-load the nodes that are connected to
// the "titles" edge. The optional arguments are used to configure the query builder of the edge.
func (eq *EmployeeQuery) WithTitles(opts ...func(*TitleQuery)) *EmployeeQuery {
	query := (&TitleClient{config: eq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (eq *EmployeeQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Employee, error) {
	var (
		...
		_spec       = eq.querySpec()
		loadedTypes = [4]bool{
			eq.withDeptEmployees != nil,
			eq.withDeptManagers != nil,
			eq.withSalaries != nil,
			eq.withTitles != nil,
		}
	)
	...
	if query := eq.withDeptEmployees; query != nil {
		if err := eq.loadDeptEmployees(ctx, query, nodes,
			func(n *Employee) { n.Edges.DeptEmployees = []*DeptEmployee{} },
			func(n *Employee, e *DeptEmployee) { n.Edges.DeptEmployees = append(n.Edges.DeptEmployees, e) }); err != nil {
			return nil, err
		}
	}
	if query := eq.withDeptManagers; query != nil {
		if err := eq.loadDeptManagers(ctx, query, nodes,
			func(n *Employee) { n.Edges.DeptManagers = []*DeptManager{} },
			func(n *Employee, e *DeptManager) { n.Edges.DeptManagers = append(n.Edges.DeptManagers, e) }); err != nil {
			return nil, err
		}
	}
	if query := eq.withSalaries; query != nil {
		if err := eq.loadSalaries(ctx, query, nodes,
			func(n *Employee) { n.Edges.Salaries = []*Salary{} },
			func(n *Employee, e *Salary) { n.Edges.Salaries = append(n.Edges.Salaries, e) }); err != nil {
			return nil, err
		}
	}
	if query := eq.withTitles; query != nil {
		if err := eq.loadTitles(ctx, query, nodes,
			func(n *Employee) { n.Edges.Titles = []*Title{} },
			func(n *Employee, e *Title) { n.Edges.Titles = append(n.Edges.Titles, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (eq *EmployeeQuery) loadDeptEmployees(ctx context.Context, query *DeptEmployeeQuery, nodes []*Employee, init func(*Employee), assign func(*Employee, *DeptEmployee)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(deptemployee.FieldEmployeeID)
	}
	query.Where(predicate.DeptEmployee(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(employee.DeptEmployeesColumn), fks...))
	}))
...
func (eq *EmployeeQuery) loadDeptManagers(ctx context.Context, query *DeptManagerQuery, nodes []*Employee, init func(*Employee), assign func(*Employee, *DeptManager)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*Employee)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(deptmanager.FieldEmployeeID)
	}
	query.Where(predicate.DeptManager(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(employee.DeptManagersColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (eq *EmployeeQuery) loadSalaries(ctx context.Context, query *SalaryQuery, nodes []*Employee, init func(*Employee), assign func(*Employee, *Salary)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*Employee)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(salary.FieldEmployeeID)
	}
	query.Where(predicate.Salary(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(employee.SalariesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (eq *EmployeeQuery) loadTitles(ctx context.Context, query *TitleQuery, nodes []*Employee, init func(*Employee), assign func(*Employee, *Title)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*Employee)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(title.FieldEmployeeID)
	}
	query.Where(predicate.Title(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(employee.TitlesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
